          ...
          for (int32_t i = 0 ; i < undo_levels - 1 ; i++)
            {
              ...
              try
                ...
                {
                  QMessageBox::critical (0, gridPointEdit3D::tr ("gridPointEdit3D store undo"),
                                         gridPointEdit3D::tr ("Unable to allocate UNDO validity memory!  Reason : %1").arg (strerror (errno)));
                  exit (-1);
                }
              ...
              try
                ...
                {
                  QMessageBox::critical (0, gridPointEdit3D::tr ("gridPointEdit3D store undo"),
                                         gridPointEdit3D::tr ("Unable to allocate UNDO num memory!  Reason : %1").arg (strerror (errno)));
                  exit (-1);
                }
...
uint8_t resize_undo (MISC *misc, OPTIONS *options, int32_t undo_levels)
{
  ...
  if (undo_levels <= misc->undo_count)
    {
      QMessageBox::warning (0, gridPointEdit3D::tr ("gridPointEdit3D resize undo"),
                            gridPointEdit3D::tr ("Resizing to less than already existing undo blocks is not allowed!"));
      return (NVFalse);
    }
