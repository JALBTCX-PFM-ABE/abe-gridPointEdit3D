...
{
  double xyz_y = ((y - misc->chrtr2_share->edit_area.min_y) / edit_y_range) * misc->draw_area_height;
  double xyz_x = ((x - misc->chrtr2_share->edit_area.min_x) / edit_x_range) * misc->draw_area_width;
...
void compute_ortho_values (nvMapGL *map, MISC *misc, OPTIONS *options, POINT_DATA *data, QScrollBar *sliceBar, uint8_t set_to_min)
{
  ...
  edit_x_range = misc->chrtr2_share->edit_area.max_x - misc->chrtr2_share->edit_area.min_x;
  edit_y_range = misc->chrtr2_share->edit_area.max_y - misc->chrtr2_share->edit_area.min_y;
  ...
  for (int32_t i = 0 ; i < data->count ; i++)
    {
      if (!check_bounds (options, misc, data, i, misc->slice, NVFalse))
        {
          int32_t z = pseudo_dist_from_viewer (misc, data->x[i], data->y[i]);
